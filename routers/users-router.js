...
router.post('/:userId/add-review', checkForReviewData, validateUserId, async (req, res) => {
		const user = res.locals.user;

		const review = {...res.locals.newReview, user_id: user.id};

		if (res.locals.authorizedUser.id === user.id) { // Create middleware to check for self
			try {
...
router.put('/:userId/reviews/:revId', validateUserId, validateReviewId, checkForReviewData, async (req, res) => {
	try  {
		const updatedReview = await Revs.updateReview(res.locals.review.id, res.locals.newReview);
...
router.delete('/:userId/reviews/:revId', validateUserId, validateReviewId, async (req, res) => {
	try {
